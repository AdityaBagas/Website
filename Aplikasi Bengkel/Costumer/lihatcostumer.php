    ...
    if(isset($_POST['submit'])) {
       // $id_sparepart = $_POST['id_sparepart'];
        ...
        if( $nama == "" || $telp == ""|| $alamat == "" ) {
            ...
            <br/>";
            <center><font size="20"><a href='lihatcostumer.php'>Go back</a></font></center>;
            <?php
        } 
        else {
            mysqli_query($conn, "INSERT INTO costumer(nama,telp,alamat) 
            VALUES('$nama','$telp','$alamat')")
            or die(mysqli_error($conn));
            
            ?>
            <center><font size="30">regist costumer succes</font></center>
            <br/>";
            <center><font size="20"><a href='lihatcostumer.php'>Go back</a></font></center>;
            <?php
        }
...
<table border="1">
	...
	if(isset($_GET['cari'])){
		$cari = $_GET['cari'];
		$data = mysqli_query($conn,"select * from costumer where CONCAT(nama,alamat) like '%".$cari."%'");				
	}else{
		$data = mysqli_query($conn,"select * from costumer");		
	}
	while($d = mysqli_fetch_array($data)){
	?>
	<tr>
		<td><?php echo $d['id_costumer']; ?></td>
        <td><?php echo $d['nama']; ?></td>
        <td><?php echo $d['telp']; ?></td>
        <td><?php echo $d['alamat']; ?></td>
        <td><?php echo "<a href=\"editcostumer.php?id_costumer=$d[id_costumer]\">Edit</a> | 
        <a href=\"deletecostumer.php?id_costumer=$d[id_costumer]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a>"?></td>;
	</tr>
	<?php } ?>
</table>   
<footer id="footer">
